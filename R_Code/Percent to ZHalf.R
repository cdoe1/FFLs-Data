...
loop2 = "C1_AND_10x"
loop3 = "C1_AND_SRG"
loop4 = "C1_AND_SRG_10x"
dir1 = "../Results/C1_AND"
dir2 = "../Results/C1_AND_10x"
dir3 = "../Results/C1_AND_SRG"
dir4 = "../Results/C1_AND_SRG_10x"
library(dplyr)
library(magrittr)
library(ggplot2)
...
for (file in files1){
  ...
  for (i in 1:1000) {
    ...
    t <- subset(t,t$Time<=50)
    m <- max(t$Z.ParticleNumber)
    b <- t[min(which(t$Z.ParticleNumber==m)),]
    Zmax <- rbind(Zmax,b)
  }
  for (i in 1:1000){
    if(Zmax$Z.ParticleNumber[i]>(SteadyMean1/2))
      p=p+1
  }
  PercentZHalf <- p/10
...
ld<-ggplot()
hd<-ggplot()
ld<-ld+geom_point(data=zhalfs1, size=4, aes(x=zhalf_times1,y=zhalf_percents1,colour = "CFFL1",shape="CFFL1"),stroke=1)
ld<-ld+geom_point(data=zhalfs3, size=4, aes(x=zhalf_times3,y=zhalf_percents3,colour = "SRG",shape="SRG"),stroke=1)

hd<-hd+geom_point(data=zhalfs2, size=4, aes(x=zhalf_times2,y=zhalf_percents2,colour = "CFFL1 10x",shape="CFFL1 10x"),stroke=1)
...
ld + theme(plot.title = element_text(hjust = 0.5)) + ylim(0, 100)+theme_bw()+
  xlab("Duration of Signal Presence (min)") + ylab("Probability of 'Adequate Response'")
