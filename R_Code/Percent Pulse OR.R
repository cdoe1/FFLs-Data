...
SteadyData <- read.csv(file="C1_OR_SRG_10x_SteadyYnoX.csv")
...
# Trims each trial to first 50 minutes to see if a pulse occurred
# within the first 50 minutes
p <- 0
Zmin <- data.frame()
for (i in 1:1000) {
  t <- subset(RawData,RawData$Trial==i)
  t <- subset(t,t$Time<=(250+signalDelay))
  m <- min(t$Z.ParticleNumber)
  b <- t[min(which(t$Z.ParticleNumber==m)),]
  Zmin <- rbind(Zmin,b)
}
for (i in 1:1000){
  if(Zmin$Z.ParticleNumber[i]<(SteadyMean-2*SteadySD))
    p=p+1
}
PercentPulse <- p/10
